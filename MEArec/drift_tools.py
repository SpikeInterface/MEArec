...
def generate_drift_dict_from_params(
    ...
    if external_drift_vector_um is None:
        ...
        t_start_drift = 0. if t_start_drift is None else t_start_drift
        ...
        if external_drift_times is None:
            assert len(external_drift_vector_um) == n_samples, \
            ...
            assert len(external_drift_times) == len(external_drift_vector_um), \
        ...
        if drift_mode_probe == "rigid":
            ...
            if non_rigid_gradient_mode == "linear":
                ...
                if non_rigid_step_depth_boundary is None:
                    ...
                    assert np.min(proj) < non_rigid_step_depth_boundary < np.max(proj), \
                ...
                if non_rigid_step_factors is None:
                    ...
                    assert 0 <= non_rigid_step_factors[0] <= 1, \
                        "'non_rigid_step_factors' needs to be in the [0, 1] range"
                    assert 0 <= non_rigid_step_factors[1] <= 1, \
                        "'non_rigid_step_factors' needs to be in the [0, 1] range"
