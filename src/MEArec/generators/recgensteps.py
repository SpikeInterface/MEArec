...
def chunk_apply_filter_(ch, i_start, i_stop, fs, lsb, recordings, pad_samples, cutoff, order, dtype):
    ...
    if cutoff.size == 1:
        filtered_chunk = filter_analog_signals(
            recordings[i_start_pad:i_stop_pad], freq=cutoff, fs=fs, filter_type="highpass", order=order
        )
    elif cutoff.size == 2:
        if fs / 2.0 < cutoff[1]:
            filtered_chunk = filter_analog_signals(
                recordings[i_start_pad:i_stop_pad], freq=cutoff[0], fs=fs, filter_type="highpass", order=order
            )
        else:
